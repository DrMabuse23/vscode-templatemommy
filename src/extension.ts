...
export function activate(context : vscode.ExtensionContext) {
    ...
    let disposable = vscode
        .commands
        .registerCommand('extension.mommy', (etc) => {
            console.log(etc);
            const a = etc;
            let folder : string;
            let file : string;
            const handleError = (err) => vscode
                .window
                .showErrorMessage(err.message, err);
            vscode
                .window
                .showQuickPick(['Module'])
                .then((res) => {
                    const e = res;
                    return vscode
                        .window
                ...
                .then((value) => {
                    ...
                    return new Promise((resolve, reject) => fs.mkdir(`${root}/${folder}`, (err) => err
                        ? reject(err)
                        : resolve(file)));
                })
                .then((file) => {
                    return new Promise((resolve, reject) => fs.writeFile(`${root}/${folder}/${file}.ts`, `export class ${file} extends Component {} `, err => err
                        ? reject(err)
                        : resolve(file)))
                })
                .then(res => vscode.window.showInformationMessage(`Module ${file} is created.`), handleError);
        });
